...
public class QuadTree {
	    ...
	    public boolean Include(Node node, TreeNode treenode) {
			if(treenode.getBox().getMinX()<=node.getX() && treenode.getBox().getMaxX()>=node.getX() && treenode.getBox().getMinY()<=node.getY() && treenode.getBox().getMaxY()>=node.getY()) {
				return true;
			}
			else{return false;}
		}
		...
		public void MakeQuadTree(ShapeList shapelist) {
			...
			for(int i=0;i<shapelist.size();i++) {
				...
				while(currentTN!=null){
					...
					if(currentTN.getChild()[0]!=null && currentTN.getChild()[1]!=null && currentTN.getChild()[2]!=null && currentTN.getChild()[3]!=null) {
